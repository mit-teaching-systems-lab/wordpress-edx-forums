...
class BBP_Forums_Group_Extension extends BP_Group_Extension {
	...
	// This function always creates a group forum. The user doesn't get to choose whether they want it or not (the tab is hidden).
	public function create_screen_save_notab( $group_id = 0 ) {
		...
		// We always set this true, because we always create a forum (the user doesn't get to choose whether they want it or not anymore)
                $create_forum = true; //!empty( $_POST['bbp-create-group-forum'] ) ? true : false;
                $forum_id     = 0;
                $forum_ids    = bbp_get_group_forum_ids( $group_id );
