		...
		while (  $support_query->have_posts() ) :  $support_query->the_post();
			...
			if ( ! empty( $forum_id ) ) {
				...
				if ( ! in_array( $forum_support_status, array( 1, 2 ) ) ) {
					// og: this seems to be a bug that makes the plugin delete the support status when the dashboard is accessed.
					// The goal here is not to allow forum status updates for forums that have their sttings set to "This forum does not accept support topics", 3
					// The problem is that if a forum was created before this plugin was activated, its _bpbbpst_forum_settings will be empty
					// Unlike forum support status 3, empty settings still enable the user to change the support forum type, 
					// so we want to make sure it doesn't get deleted here
					// We treat empty settings like 1 or 2, not 3
					// http://php.net/manual/en/function.empty.php
					if(!empty($forum_support_status)){
						// Let's avoid the display of the topic in the WP_List_Table	
						delete_post_meta( $support_query->post->ID, '_bpbbpst_support_topic' );
						//echo "support status ".$forum_support_status." ".print_r($forum_support_status,true);
						continue;
					}
				}
			}
